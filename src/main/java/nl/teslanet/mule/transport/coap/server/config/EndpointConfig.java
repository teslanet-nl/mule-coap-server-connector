...
public class EndpointConfig {
	...
	public NetworkConfig getNetworkConfig() {
		...
		if (this.bindToPort != null)
			config.setInt(NetworkConfig.Keys.COAP_PORT, this.bindToPort);
		if (this.bindToSecurePort != null)
			config.setInt(NetworkConfig.Keys.COAP_SECURE_PORT, this.bindToSecurePort);
		if (this.maxActivePeers != null)
			config.setInt(NetworkConfig.Keys.MAX_ACTIVE_PEERS, this.maxActivePeers);
		if (this.maxPeerInactivityPeriod != null)
			config.setInt(NetworkConfig.Keys.MAX_PEER_INACTIVITY_PERIOD, this.maxPeerInactivityPeriod);
		if (this.ackTimeout != null)
			config.setInt(NetworkConfig.Keys.ACK_TIMEOUT, this.ackTimeout); // 2000);
		if (this.ackRandomFactor != null)
			config.setFloat(NetworkConfig.Keys.ACK_RANDOM_FACTOR, this.ackRandomFactor); // 1.5f); Float.va
		if (this.ackTimeoutScale != null)
			config.setFloat(NetworkConfig.Keys.ACK_TIMEOUT_SCALE, this.ackTimeoutScale); // 2f);
		if (this.maxRetransmit != null)
			config.setInt(NetworkConfig.Keys.MAX_RETRANSMIT, this.maxRetransmit); // 4);
		if (this.exchangeLifetime != null)
			config.setLong(NetworkConfig.Keys.EXCHANGE_LIFETIME, this.exchangeLifetime); // 247 * 1000); // ms
		if (this.nonLifetime != null)
			config.setLong(NetworkConfig.Keys.NON_LIFETIME, this.nonLifetime); // 145 * 1000); // ms
		if (this.maxTransmitWait != null)
			config.setLong(NetworkConfig.Keys.MAX_TRANSMIT_WAIT, this.maxTransmitWait); // 93 * 1000);
		if (this.nstart != null)
			config.setInt(NetworkConfig.Keys.NSTART, this.nstart); // 1);
		if (this.leisure != null)
			config.setInt(NetworkConfig.Keys.LEISURE, this.leisure); // 5000);
		if (this.probingRate != null)
			config.setFloat(NetworkConfig.Keys.PROBING_RATE, this.probingRate); // 1f);

		if (this.useRandomMidStart != null)
			config.setBoolean(NetworkConfig.Keys.USE_RANDOM_MID_START, this.useRandomMidStart); // true);
		...
		if (this.midTrackerGroups != null)
			config.setInt(NetworkConfig.Keys.MID_TRACKER_GROUPS, this.midTrackerGroups);
		if (this.tokenSizeLimit != null)
			config.setInt(NetworkConfig.Keys.TOKEN_SIZE_LIMIT, this.tokenSizeLimit); // 8);

		if (this.preferredBlockSize != null)
			config.setInt(NetworkConfig.Keys.PREFERRED_BLOCK_SIZE, this.preferredBlockSize); // 512);
		if (this.maxMessageSize != null)
			config.setInt(NetworkConfig.Keys.MAX_MESSAGE_SIZE, this.maxMessageSize); // 1024);
		// TODO: only transparent blockwise is supported: maxResourceBodySize > 0
		if (this.maxResourceBodySize != null && this.maxResourceBodySize > 0)
			config.setInt(NetworkConfig.Keys.MAX_RESOURCE_BODY_SIZE, this.maxResourceBodySize); // 8192 bytes);
		if (this.blockwiseStatusLifetime != null)
			config.setInt(NetworkConfig.Keys.BLOCKWISE_STATUS_LIFETIME, this.blockwiseStatusLifetime); // 5 * 60 *
																										// 1000); // ms
																										// (5min)

		if (this.notificationCheckIntervalTime != null)
			config.setLong(NetworkConfig.Keys.NOTIFICATION_CHECK_INTERVAL_TIME, this.notificationCheckIntervalTime); // 24
																														// *
																														// 60
																														// *
																														// 60
		...
		if (this.notificationCheckIntervalCount != null)
			config.setInt(NetworkConfig.Keys.NOTIFICATION_CHECK_INTERVAL_COUNT, this.notificationCheckIntervalCount); // 100);
		if (this.notificationReregistrationBackoff != null)
			config.setLong(NetworkConfig.Keys.NOTIFICATION_REREGISTRATION_BACKOFF,
					this.notificationReregistrationBackoff); // 2000); // ms

		if (this.useCongestionControl != null)
			config.setBoolean(NetworkConfig.Keys.USE_CONGESTION_CONTROL, this.useCongestionControl); // false);
		...
		if (this.protocolStageThreadCount != null)
			config.setInt(NetworkConfig.Keys.PROTOCOL_STAGE_THREAD_COUNT, this.protocolStageThreadCount); // CORES);
		if (this.networkStageReceiverThreadCount != null)
			config.setInt(NetworkConfig.Keys.NETWORK_STAGE_RECEIVER_THREAD_COUNT, this.networkStageReceiverThreadCount); // WINDOWS
																															// ?
																															// CORES
																															// :
																															// 1);
		if (this.networkStageSenderThreadCount != null)
			config.setInt(NetworkConfig.Keys.NETWORK_STAGE_SENDER_THREAD_COUNT, this.networkStageSenderThreadCount); // WINDOWS
																														// ?
																														// CORES
																														// :
																														// 1);

		if (this.udpConnectorDatagramSize != null)
			config.setInt(NetworkConfig.Keys.UDP_CONNECTOR_DATAGRAM_SIZE, this.udpConnectorDatagramSize); // 2048);
		if (this.udpConnectorReceiveBuffer != null)
			config.setInt(NetworkConfig.Keys.UDP_CONNECTOR_RECEIVE_BUFFER, this.udpConnectorReceiveBuffer); // UDPConnector.UNDEFINED);
		if (this.udpConnectorSendBuffer != null)
			config.setInt(NetworkConfig.Keys.UDP_CONNECTOR_SEND_BUFFER, this.udpConnectorSendBuffer); // UDPConnector.UNDEFINED);
		if (this.udpConnectorOutCapacity != null)
			config.setInt(NetworkConfig.Keys.UDP_CONNECTOR_OUT_CAPACITY, this.udpConnectorOutCapacity); // Integer.MAX_VALUE);
																										// // unbounded
		...
		if (this.markAndSweepInterval != null)
			config.setLong(NetworkConfig.Keys.MARK_AND_SWEEP_INTERVAL, this.markAndSweepInterval); // 10 * 1000);
		if (this.cropRotationPeriod != null)
			config.setInt(NetworkConfig.Keys.CROP_ROTATION_PERIOD, this.cropRotationPeriod); // 2000);
		...
		if (this.secureSessionTimeout != null)
			config.setInt(NetworkConfig.Keys.SECURE_SESSION_TIMEOUT, this.secureSessionTimeout);
		if (this.dtlsAutoResumeTimeout != null)
			config.setInt(NetworkConfig.Keys.DTLS_AUTO_RESUME_TIMEOUT, this.dtlsAutoResumeTimeout);
		return config;
	}
}
